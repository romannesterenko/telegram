...
$tg = \Phabel\Target\Php80\Polyfill::fsockopen('tcp://149.154.167.51:443');
fwrite($tg, chr(239));
stream_set_timeout($tg, 1);
$tot = 100;
for ($x = 0; $x < $tot; $x++) {
    fwrite($tg, gen_payload());
    $number = \danog\PHP\Struct::unpack('>q', \Phabel\Target\Php80\Polyfill::substr(stream_get_contents($tg, 85), 58, 8))[0];
    test_single($number);
}
fclose($tg);
