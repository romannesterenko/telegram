...
class Installer
{
    ...
    private static function extractVersions($release)
    {
        ...
        foreach ($composer['packages'] as $dep) {
            ...
            if ($name === 'danog/madelineproto' && \Phabel\Target\Php80\Polyfill::substr($version, 0, 2) === '90') {
    ...
    private static function reportComposer($local_release, $remote_release)
    {
        ...
        $opts = ['http' => ['method' => 'POST', 'header' => ['Content-Type: application/json', \sprintf('User-Agent: Composer/%s (%s; %s; %s; %s%s)', 'MProto v7', \function_exists('php_uname') ? @\php_uname('s') : 'Unknown', \function_exists('php_uname') ? @\php_uname('r') : 'Unknown', $phpVersion, 'streams', \getenv('CI') ? '; CI' : '')], 'content' => \json_encode($postData), 'timeout' => 6]];
