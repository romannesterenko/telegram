...
$docs = [['tl_schema' => ['mtproto' => '', 'telegram' => "{$d}/schemas/TL_telegram_v{$layer}.tl", 'secret' => "{$d}/schemas/TL_secret.tl", 'td' => "{$d}/schemas/TL_td.tl"], 'title' => "MadelineProto API documentation (layer {$layer})", 'description' => "MadelineProto API documentation (layer {$layer})", 'output_dir' => "{$d}/docs/docs/API_docs", 'template' => "{$d}/docs/template", 'readme' => false]];
$docs = \array_merge($docs, initDocs($schemas));
$logger->logger('Creating annotations...', Logger::NOTICE);
$doc = new \danog\MadelineProto\AnnotationsBuilder($logger, $docs[0], \dirname(__FILE__) . '/../src/danog/MadelineProto/InternalDoc.php', ['API' => API::class, 'APIFactory' => APIFactory::class, 'MTProto' => MTProto::class], 'danog\\MadelineProto');
$doc->mkAnnotations();
...
$orderedfiles = [];
$order = ['CREATING_A_CLIENT', 'LOGIN', 'FEATURES', 'REQUIREMENTS', 'INSTALLATION', 'UPDATES', 'DATABASE', 'SETTINGS', 'SELF', 'EXCEPTIONS', 'FLOOD_WAIT', 'LOGGING', 'CALLS', 'FILES', 'CHAT_INFO', 'DIALOGS', 'INLINE_BUTTONS', 'SECRET_CHATS', 'PROXY', 'ASYNC', 'USING_METHODS', 'CONTRIB', 'TEMPLATES'];
$index = '';
...
foreach ($orderedfiles as $key => $filename) {
    ...
    \array_unshift($lines, '---', 'title: "' . $title . '"', 'description: "' . $description . '"', 'nav_order: ' . ($key + 4), 'image: https://docs.madelineproto.xyz/favicons/android-chrome-256x256.png', '---');
    ...
    if (\basename($filename) !== 'FEATURES.md') {
        foreach ($matches[1] as $key => $match) {
            ...
            } elseif (\Phabel\Target\Php80\Polyfill::substr($matches[3][$key], 0, 3) === '../') {
                $url = 'https://docs.madelineproto.xyz/' . \str_replace('.md', '.html', \Phabel\Target\Php80\Polyfill::substr($matches[3][$key], 3));
            } else {
                $url = $matches[3][$key];
            }
