...
trait PeerHandler
{
            ...
            case 'user':
                if (!$existingChat || $existingChat !== $user) {
                    ...
                    if (!$this->getSettings()->getDb()->getEnablePeerInfoDb()) {
                        $user = ['_' => $user['_'], 'id' => $user['id'], 'access_hash' => $user['access_hash'] ?? null, 'min' => $user['min'] ?? false];
                    }
        ...
        switch ($chat['_']) {
            ...
            case 'chatForbidden':
                ...
                if (!$existingChat || $existingChat !== $chat) {
                    ...
                    if (!$this->getSettings()->getDb()->getEnablePeerInfoDb()) {
                        $chat = ['_' => $chat['_'], 'id' => $chat['id'], 'access_hash' => $chat['access_hash'] ?? null, 'min' => $chat['min'] ?? false];
                    }
    ...
    public function getId($id)
    {
        if (\is_array($id)) {
            switch ($id['_']) {
                ...
                case 'message':
                case 'messageService':
                    if (!isset($id['from_id']) || $id['peer_id']['_'] !== 'peerUser' || $id['peer_id']['user_id'] !== $this->authorization['user']['id']) {
    ...
    */
    public function getInfo($id, int $type = 3, $recursive = true) : \Generator
    {
        if (\is_array($id)) {
            switch ($id['_']) {
            ...
            if ((yield $this->chats[$id])) {
                ...
                try {
                    ...
                    if ($e->getMessage() === 'This peer is not present in the internal peer database') {
                        (yield $this->chats->unset($id));
                        /** @uses DbArray::offsetUnset() */
                    } else {
                        throw $e;
                    }
                }
    ...
    */
    ...
    public function getPwrChat($id, bool $fullfetch = true, bool $send = true) : \Generator
    {
        ...
        if (isset($res['photo'])) {
            $photo = [];
            foreach (['small' => $res['photo']['sizes'][0], 'big' => Tools::maxSize($res['photo']['sizes'])] as $type => $size) {
                $fileId = new FileId();
