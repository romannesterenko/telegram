    ...
    public function unencryptedWriteLoop() : \Generator
    {
        ...
        while ($connection->pendingOutgoing) {
            ...
            foreach ($connection->pendingOutgoing as $k => $message) {
                ...
                (yield $buffer->bufferWrite("\x00\x00\x00\x00\x00\x00\x00\x00" . $message_id . \danog\MadelineProto\Tools::packUnsignedInt($length) . $message->getSerializedBody() . $pad));
                $connection->httpSent();
    ...
    public function encryptedWriteLoop() : \Generator
    {
        ...
        do {
            ...
            foreach ($connection->pendingOutgoing as $k => $message) {
                ...
                $MTmessage = ['_' => 'MTmessage', 'msg_id' => $message_id, 'body' => $message->getSerializedBody(), 'seqno' => $message->getSeqNo() ?? $connection->generateOutSeqNo($message->isContentRelated())];
                ...
                $messages[] = $MTmessage;
                $keys[$k] = $message_id;
                $message->setSeqNo($MTmessage['seqno'])->setMsgId($MTmessage['msg_id']);
            }
            $MTmessage = null;
            ...
            if ($ackCount = \count($acks)) {
                ...
                $messages[] = ['_' => 'MTmessage', 'msg_id' => $connection->msgIdHandler->generateMessageId(), 'body' => $body, 'seqno' => $connection->generateOutSeqNo(false), 'bytes' => \strlen($body)];
                $count++;
                unset($acks, $body);
            }
            if ($shared->isHttp() && !$has_http_wait) {
                ...
                $messages[] = ['_' => 'MTmessage', 'msg_id' => $connection->msgIdHandler->generateMessageId(), 'body' => $body, 'seqno' => $connection->generateOutSeqNo(true), 'bytes' => \strlen($body)];
                $count++;
                unset($body);
            }
            if ($count > 1 || $has_seq) {
            ...
            $message_key = \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($shared->getTempAuthKey()->getAuthKey(), 88, 32) . $plaintext . $padding, true), 8, 16);
