...
class Ogg
{
    ...
    public static function init(BufferedStreamInterface $stream, int $frameDuration) : \Generator
    {
        ...
        $pack_format = ['stream_structure_version' => 'C', 'header_type_flag' => 'C', 'granule_position' => 'P', 'bitstream_serial_number' => 'V', 'page_sequence_number' => 'V', 'CRC_checksum' => 'V', 'number_page_segments' => 'C'];
        $self->packFormat = \implode('/', \array_map(fn(string $v, string $k): string => ($v . $k), $pack_format, \array_keys($pack_format)));
        return $self;
    }
    /**
     * Read OPUS length.
    ...
    private function opusStateMachine(string $content) : \Generator
    {
        ...
        while ($offset < $len) {
            ...
            if ($c === 0) {
                // Exactly 1 frame
                $sizes[] = $selfDelimited ? $this->readLen($content, $offset) : $len - $offset;
            } elseif ($c === 1) {
                // Exactly 2 frames, equal size
                $size = $selfDelimited ? $this->readLen($content, $offset) : ($len - $offset) / 2;
                $sizes[] = $size;
                $sizes[] = $size;
            } elseif ($c === 2) {
                ...
                $sizes[] = $size;
                $sizes[] = $selfDelimited ? $this->readLen($content, $offset) : $len - ($offset + $size);
            } else {
                ...
                if ($vbr) {
                    ...
                    // CBR
                    $size = $selfDelimited ? $this->readLen($content, $offset) : ($len - ($offset + $padding)) / $count;
            ...
            if (!$selfDelimited && $totalDuration + $this->currentDuration <= $this->frameDuration) {
                ...
                $sum = \array_sum($sizes);
                $this->opusPayload .= \Phabel\Target\Php80\Polyfill::substr($content, $preOffset, $offset - $preOffset + $sum + $paddingLen);
            ...
            foreach ($sizes as $size) {
                ...
                $this->opusPayload .= \Phabel\Target\Php80\Polyfill::substr($content, $offset, $size);
                $offset += $size;
    ...
    public function read() : \Generator
    {
        ...
        while (true) {
            ...
            if (\Phabel\Target\Php80\Polyfill::substr($init, 0, 4) !== self::CAPTURE_PATTERN) {
            ...
            /*$headers = \unpack(
                        ...
                        */
            $segments = \unpack('C*', (yield $this->stream->bufferRead(\ord($init[26]))));
            ...
            foreach ($segments as $segment_size) {
                ...
                if ($segment_size < 255) {
                    ...
                    if ($state === self::STATE_STREAMING) {
                        ...
                        if (\Phabel\Target\Php80\Polyfill::substr($content, 0, 8) !== 'OpusHead') {
                            throw new \RuntimeException("This is not an OPUS stream!");
                        }
                        $opus_head = \unpack('Cversion/Cchannel_count/vpre_skip/Vsample_rate/voutput_gain/Cchannel_mapping_family/', \Phabel\Target\Php80\Polyfill::substr($content, 8));
                        if ($opus_head['channel_mapping_family']) {
                            $opus_head['channel_mapping'] = \unpack('Cstream_count/Ccoupled_count/C*channel_mapping', \Phabel\Target\Php80\Polyfill::substr($content, 19));
                        } else {
                            $opus_head['channel_mapping'] = ['stream_count' => 1, 'coupled_count' => $opus_head['channel_count'] - 1, 'channel_mapping' => [0]];
                        ...
                        $vendor_string_length = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($content, 8, 4))[1];
                        $result = [];
                        $result['vendor_string'] = \Phabel\Target\Php80\Polyfill::substr($content, 12, $vendor_string_length);
                        $comment_count = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($content, 12 + $vendor_string_length, 4))[1];
                        $offset = 16 + $vendor_string_length;
                        for ($x = 0; $x < $comment_count; $x++) {
                            $length = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($content, $offset, 4))[1];
                            $result['comments'][$x] = \Phabel\Target\Php80\Polyfill::substr($content, $offset += 4, $length);
                            $offset += $length;
                        }
