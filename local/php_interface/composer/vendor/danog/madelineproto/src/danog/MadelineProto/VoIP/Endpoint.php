...
class Endpoint
{
    ...
    public function read() : \Generator
    {
        do {
            ...
            if (\stream_get_contents($payload, 12) === "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00") {
                ...
                if ($message_key != \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($this->authKey->getAuthKey(), 88 + ($this->creator ? 8 : 0), 32) . $packet, true), 8, 16)) {
                    \danog\MadelineProto\Logger::log("msg_key mismatch!", \danog\MadelineProto\Logger::ERROR);
                    return false;
                }
                $innerLen = \unpack('v', \Phabel\Target\Php80\Polyfill::substr($packet, 0, 2))[1];
                ...
                $packet = \Phabel\Target\Php80\Polyfill::substr($packet, 2);
            }
    ...
    public function write(string $payload) : Promise
    {
        ...
        $message_key = \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($this->authKey->getAuthKey(), 88 + ($this->creator ? 0 : 8), 32) . $plaintext, true), 8, 16);
