    ...
    public function readMessage() : \Generator
    {
        ...
        try {
            ...
            if (\strpos($e->getReason(), '       ') === 0) {
                $payload = -\Phabel\Target\Php80\Polyfill::substr($e->getReason(), 7);
        ...
        try {
            ...
            if ($auth_key_id === "\x00\x00\x00\x00\x00\x00\x00\x00") {
                ...
                if ($message_key != \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($shared->getTempAuthKey()->getAuthKey(), 96, 32) . $decrypted_data, true), 8, 16)) {
                ...
                */
                $session_id = \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 8, 8);
                if ($session_id !== $connection->session_id) {
                    $API->logger->logger("Session ID mismatch", Logger::FATAL_ERROR);
                    $connection->resetSession();
                    throw new NothingInTheSocketException();
                }
                $message_id = \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 16, 8);
                $connection->msgIdHandler->checkMessageId($message_id, ['outgoing' => false, 'container' => false]);
                $seq_no = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 24, 4))[1];
                $message_data_length = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 28, 4))[1];
                ...
                $message_data = \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 32, $message_data_length);
            } else {
