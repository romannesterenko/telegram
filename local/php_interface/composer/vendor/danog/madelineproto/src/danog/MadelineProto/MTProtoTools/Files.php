    ...
    */
    public function uploadFromCallable(callable $callable, int $size, string $mime, string $fileName = '', $cb = NULL, bool $seekable = true, bool $encrypted = false) : \Generator
    {
        ...
        if ($encrypted === true) {
            ...
            $fingerprint = Tools::unpackSignedInt(\Phabel\Target\Php80\Polyfill::substr($digest, 0, 4) ^ \Phabel\Target\Php80\Polyfill::substr($digest, 4, 4));
        ...
        switch ($messageMedia['_']) {
            ...
            case 'messageMediaPhoto':
                ...
                if (isset($size['volume_id'])) {
                    $res['InputFileLocation'] = ['_' => 'inputPhotoLegacyFileLocation', 'id' => $messageMedia['id'], 'access_hash' => $messageMedia['access_hash'], 'dc_id' => $messageMedia['dc_id'], 'file_reference' => yield from $this->referenceDatabase->getReference(ReferenceDatabase::PHOTO_LOCATION, $messageMedia), 'volume_id' => $size['volume_id'], 'local_id' => $size['local_id'], 'secret' => $size['secret']];
                } else {
                    $res = \array_merge($res, yield from $this->getDownloadInfo($size));
                    $res['InputFileLocation'] = ['_' => 'inputPhotoFileLocation', 'id' => $messageMedia['id'], 'access_hash' => $messageMedia['access_hash'], 'dc_id' => $messageMedia['dc_id'], 'file_reference' => yield from $this->referenceDatabase->getReference(ReferenceDatabase::PHOTO_LOCATION, $messageMedia), 'thumb_size' => $res['thumb_size'] ?? 'x'];
                }
            ...
            case 'updateUserPhoto':
                ...
                $res['InputFileLocation'] = ['_' => 'inputPeerPhotoFileLocation', 'big' => true, 'peer' => $peer, 'photo_id' => $res['InputFileLocation']['photo_id']];
    ...
    */
    ...
    private function downloadPart(&$messageMedia, bool &$cdn, &$datacenter, &$old_dc, &$ige, $cb, array $offset, $callable, bool $seekable, bool $postpone = false) : \Generator
    {
        ...
        do {
            ...
            if (isset($messageMedia['cdn_key'])) {
                $ivec = \Phabel\Target\Php80\Polyfill::substr($messageMedia['cdn_iv'], 0, 12) . \pack('N', $offset['offset'] >> 4);
            ...
            if ($offset['part_start_at'] || $offset['part_end_at'] !== $offset['limit']) {
                $res['bytes'] = \Phabel\Target\Php80\Polyfill::substr($res['bytes'], $offset['part_start_at'], $offset['part_end_at'] - $offset['part_start_at']);
            }
            if (!$seekable) {
    ...
    private function checkCdnHash($file, $offset, $data, &$datacenter) : \Generator
    {
        while (\strlen($data)) {
            ...
            if (\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($data, 0, $this->cdn_hashes[$file][$offset]['limit']), true) !== $this->cdn_hashes[$file][$offset]['hash']) {
                throw new \danog\MadelineProto\SecurityException('CDN hash mismatch for offset ' . $offset);
            }
            $data = \Phabel\Target\Php80\Polyfill::substr($data, $this->cdn_hashes[$file][$offset]['limit']);
