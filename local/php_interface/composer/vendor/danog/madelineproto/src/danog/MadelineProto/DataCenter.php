        ...
        if ($reconnectAll || $changedSettings || !$this->CookieJar) {
            $configProvider = new class implements ConfigLoader
            {
                ...
                private function loadConfigGenerator() : \Generator
                {
                    ...
                    } catch (\Throwable $phabel_03a4f6a14fcc5562) {
                        return new Config(['1.1.1.1', '1.0.0.1', '[2606:4700:4700::1111]', '[2606:4700:4700::1001]']);
                    }
                }
                /**
                 *
            ...
            $this->DoHClient = Magic::$altervista || Magic::$zerowebhost || !$settings->getUseDoH() ? new Rfc1035StubResolver(null, $configProvider) : new Rfc8484StubResolver($DoHConfig);
            $this->nonProxiedDoHClient = Magic::$altervista || Magic::$zerowebhost || !$settings->getUseDoH() ? new Rfc1035StubResolver(null, $configProvider) : new Rfc8484StubResolver($nonProxiedDoHConfig);
    ...
    public function generateContexts($dc_number = 0, string $uri = '', ConnectContext $context = null) : array
    {
        ...
        foreach ($combos as $combo) {
            foreach ([true, false] as $useDoH) {
                $ipv6Combos = [$this->settings->getIpv6() ? 'ipv6' : 'ipv4', $this->settings->getIpv6() ? 'ipv4' : 'ipv6'];
