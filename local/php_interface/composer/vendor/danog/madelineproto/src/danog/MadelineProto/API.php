...
class API extends InternalDoc
{
    ...
    {
        Logger::constructorFromSettings($settings instanceof Settings ? $settings->getLogger() : ($settings instanceof SettingsLogger ? $settings : new SettingsLogger()));
    ...
    protected function connectToMadelineProto(SettingsAbstract $settings, bool $forceFull = false, bool $tryReconnect = true) : \Generator
    {
        ...
        [$unserialized, $this->unlock] = (yield Tools::timeoutWithDefault(Serialization::unserialize($this->session, $settings, $forceFull), 30000, [0, null]));
        if ($unserialized === 0) {
            // Timeout
