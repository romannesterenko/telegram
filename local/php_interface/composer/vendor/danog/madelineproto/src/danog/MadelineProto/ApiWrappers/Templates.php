...
trait Templates
{
    ...
    {
        $message = \Phabel\Target\Php80\Polyfill::htmlentities($message);
        if (!isset($this->myTelegramOrgWrapper)) {
            if (isset($_POST['type'])) {
                if ($_POST['type'] === 'manual') {
                    $title = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiManualWeb']);
                    $title .= "<br><b>{$message}</b>";
                    $title .= "<ol>";
                    $title .= "<li>" . \str_replace('https://my.telegram.org', '<a href="https://my.telegram.org" target="_blank">https://my.telegram.org</a>', \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiManualInstructions0'])) . "</li>";
                    $title .= "<li>" . \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiManualInstructions1']) . "</li>";
                    $title .= "<li><ul>";
                    foreach (['App title', 'Short name', 'URL', 'Platform', 'Description'] as $k => $key) {
                        $title .= "<li>{$key}: ";
                        $title .= \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang["apiAppInstructionsManual{$k}"]);
                        $title .= "</li>";
                    }
                    $title .= "</li></ul>";
                    $title .= "<li>" . \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiManualInstructions2']) . "</li>";
                    $title .= "</ol>";
                    ...
                    $title .= "<br><b>{$message}</b>";
                    $phone = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginUserPhoneWeb']);
                ...
                $title = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiChooseManualAutoWeb']);
                $title .= "<br>";
                $title .= \sprintf(Lang::$current_lang['apiChooseManualAutoTipWeb'], 'https://docs.madelineproto.xyz/docs/SETTINGS.html');
                $title .= "<b>{$message}</b>";
                $automatically = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiChooseAutomaticallyWeb']);
                $manually = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiChooseManuallyWeb']);
            ...
            if (!$this->myTelegramOrgWrapper->loggedIn()) {
                $title = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginUserCode']);
                $title .= "<br><b>{$message}</b>";
                $code = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginUserPhoneCodeWeb']);
                $form = "<input type='text' name='code' placeholder='{$code}' required/>";
            } else {
                $title = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiAppWeb']);
                $title .= "<br><b>{$message}</b>";
                ...
                foreach (['app_title', 'app_shortname', 'app_url', 'app_platform', 'app_desc'] as $k => $field) {
                    $desc = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang["apiAppInstructionsAuto{$k}"]);
                    if ($field == 'app_platform') {
                        $form .= "{$desc}<br>";
                        foreach (['android' => 'Android', 'ios' => 'iOS', 'wp' => 'Windows Phone', 'bb' => 'BlackBerry', 'desktop' => 'Desktop', 'web' => 'Web', 'ubp' => 'Ubuntu phone', 'other' => \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['apiAppInstructionsAutoTypeOther'])] as $key => $desc) {
