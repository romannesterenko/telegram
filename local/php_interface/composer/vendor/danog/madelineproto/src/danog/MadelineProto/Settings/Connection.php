...
class Connection extends SettingsAbstract
{
     ...
     */
    protected array $sslSubdomains = [1 => 'pluto', 2 => 'venus', 3 => 'aurora', 4 => 'vesta', 5 => 'flora'];
    /**
     *
     */
    public function mergeArray(array $settings) : void
    {
        ...
        foreach (self::toCamel(['robin_period', 'default_dc', 'pfs']) as $object => $array) {
            if (isset($settings[$array])) {
                $this->{$object}($settings[$array]);
            }
        }
        $settings = $settings['all'] ?? [];
        foreach (self::toCamel(['test_mode', 'ipv6', 'timeout', 'obfuscated']) as $object => $array) {
            if (isset($settings[$array])) {
                $this->{$object}($settings[$array]);
            }
        }
