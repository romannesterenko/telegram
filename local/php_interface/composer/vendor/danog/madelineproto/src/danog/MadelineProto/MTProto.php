...
class MTProto extends AsyncConstruct implements TLCallback
{
    ...
    private const RSA_KEYS = ['-----BEGIN RSA PUBLIC KEY-----
...
-----END RSA PUBLIC KEY-----'];
    private const TEST_RSA_KEYS = ['-----BEGIN RSA PUBLIC KEY-----
...
-----END RSA PUBLIC KEY-----'];
    ...
    */
     ...
     * @var array
     */
    protected static array $dbProperties = ['chats' => 'array', 'full_chats' => 'array', 'sponsoredMessages' => 'array', 'channelParticipants' => 'array', 'usernames' => 'array', 'session' => ['type' => 'array', 'config' => ['enableCache' => false]]];
    /**
    ...
    private function cleanupProperties() : \Generator
    {
        ...
        if (!$this->settings->getDb()->getEnablePeerInfoDb()) {
            ...
            while ((yield $iterator->advance())) {
                ...
                $value = ['_' => $value['_'], 'id' => $value['id'], 'access_hash' => $value['access_hash'] ?? null, 'min' => $value['min'] ?? false];
                $k++;
                if ($k % 500 === 0 || $k === $total) {
    ...
    private function wakeupAsync(SettingsAbstract $settings) : \Generator
    {
        ...
        $forceDialogs = false;
        if (!isset($this->v) || $this->v !== self::V || $this->settings->getSchema()->needsUpgrade()) {
    ...
    public function updateSettings(SettingsAbstract $settings) : \Generator
    {
        ...
        if ($this->settings->getAuth()->hasChanged() || $this->settings->getConnection()->hasChanged() || $this->settings->getSchema()->hasChanged() || $this->settings->getSchema()->needsUpgrade()) {
            yield from $this->__construct_async($this->settings);
        }
    }
    /**
    ...
    {
        $this->logger = new Logger($this->settings->getLogger(), $this->authorization['user']['username'] ?? $this->authorization['user']['id'] ?? '');
    }
    /**
    ...
    {
        if ($this->authorized === self::LOGGED_IN && \class_exists(VoIPServerConfigInternal::class) && !$this->authorization['user']['bot'] && $this->datacenter->getDataCenterConnection($this->settings->getDefaultDc())->hasTempAuthKey()) {
    ...
    {
        $phabelReturn = $this->authorization['user'] ?? false;
    ...
    public function report(string $message, string $parseMode = '') : \Generator
    {
        ...
        $file = null;
        if ($this->settings->getLogger()->getType() === Logger::FILE_LOGGER && ($path = $this->settings->getLogger()->getExtra())) {
    ...
    {
        return \array_merge(\array_fill_keys(['chat', 'chatEmpty', 'chatForbidden', 'channel', 'channelEmpty', 'channelForbidden'], [[$this, 'addChat']]), \array_fill_keys(['user', 'userEmpty'], [[$this, 'addUser']]), ['help.support' => [[$this, 'addSupport']]], ['config' => [[$this, 'addConfig']]]);
    }
    /**
    ...
    */
    ...
    {
        return \array_merge(\array_fill_keys(['InputPeer'], [$this, 'getInputPeer']), \array_fill_keys(['InputUser', 'InputChannel'], [$this, 'getInputConstructor']), \array_fill_keys(['User', 'Chat', 'Peer', 'InputDialogPeer', 'InputNotifyPeer'], [$this, 'getInfo']), \array_fill_keys(['InputMedia', 'InputDocument', 'InputPhoto'], [$this, 'getFileInfo']), \array_fill_keys(['InputFileLocation'], [$this, 'getDownloadInfo']));
    }
    /**
