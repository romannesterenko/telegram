    ...
    public function mkConstructors() : void
    {
        ...
        foreach ($this->TL->getConstructors($this->td)->by_predicate_and_layer as $predicate => $id) {
            ...
            foreach ($data['params'] as $param) {
                ...
                if (\Phabel\Target\Php80\Polyfill::substr($param[$type_or_subtype], -1) === '>') {
                    $param[$type_or_subtype] = \Phabel\Target\Php80\Polyfill::substr($param[$type_or_subtype], 0, -1);
                }
                $params .= "'" . $param['name'] . "' => ";
...
';
            ...
            foreach ($data['params'] as $param) {
                ...
                if (\Phabel\Target\Php80\Polyfill::substr($ptype, -1) === '>') {
                    $ptype = \Phabel\Target\Php80\Polyfill::substr($ptype, 0, -1);
                }
                switch ($ptype) {
                    case 'true':
                    case 'false':
