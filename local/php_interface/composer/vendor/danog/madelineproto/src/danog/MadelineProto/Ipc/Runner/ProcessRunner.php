...
{
    private const CGI_VARS = ["AUTH_TYPE", "CONTENT_LENGTH", "CONTENT_TYPE", "GATEWAY_INTERFACE", "PATH_INFO", "PATH_TRANSLATED", "QUERY_STRING", "REMOTE_ADDR", "REMOTE_HOST", "REMOTE_IDENT", "REMOTE_USER", "REQUEST_METHOD", "SCRIPT_NAME", "SERVER_NAME", "SERVER_PORT", "SERVER_PROTOCOL", "SERVER_SOFTWARE"];
    ...
    public static function start(string $session, int $startupId) : Promise
    {
        ...
        $command = [$binary, ...self::formatOptions($options), $runner, 'madeline-ipc', $session, $startupId];
        ...
        $envVars = \array_merge(\array_filter($_SERVER, fn($v, $k): bool => (\is_string($v) && !\in_array($k, self::CGI_VARS)), ARRAY_FILTER_USE_BOTH), ['QUERY_STRING' => \http_build_query($params)]);
