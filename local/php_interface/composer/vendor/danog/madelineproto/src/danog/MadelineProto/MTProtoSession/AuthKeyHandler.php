    ...
    public function createAuthKey(bool $temp) : \Generator
    {
        ...
        for ($retry_id_total = 1; $retry_id_total <= $this->API->settings->getAuth()->getMaxAuthTries(); $retry_id_total++) {
            try {
                ...
                foreach (['native_single_cpp', 'python_single_alt', 'python_single', 'native_single'] as $method) {
                ...
                do {
                    ...
                    $aes_encrypted = Crypt::igeEncrypt($data_with_hash, $temp_key, \str_repeat("\x00", 32));
                 ...
                 */
                if (isset($server_dh_params['new_nonce_hash']) && \Phabel\Target\Php80\Polyfill::substr(\sha1($new_nonce), -32) != $server_dh_params['new_nonce_hash']) {
                ...
                $tmp_aes_key = \sha1($new_nonce . $server_nonce, true) . \Phabel\Target\Php80\Polyfill::substr(\sha1($server_nonce . $new_nonce, true), 0, 12);
                $tmp_aes_iv = \Phabel\Target\Php80\Polyfill::substr(\sha1($server_nonce . $new_nonce, true), 12, 8) . \sha1($new_nonce . $new_nonce, true) . \Phabel\Target\Php80\Polyfill::substr($new_nonce, 0, 4);
                $answer_with_hash = Crypt::igeDecrypt($encrypted_answer, $tmp_aes_key, $tmp_aes_iv);
                /*
                 * ***********************************************************************
                 * Separate answer and hash
                 */
                $answer_hash = \Phabel\Target\Php80\Polyfill::substr($answer_with_hash, 0, 20);
                /** @var string */
                $answer = \Phabel\Target\Php80\Polyfill::substr($answer_with_hash, 20);
                /*
                 * ***********************************************************************
                 * Deserialize answer
                ...
                if (\sha1(\Phabel\Target\Php80\Polyfill::substr($answer, 0, $server_DH_inner_data_length), true) != $answer_hash) {
                ...
                for ($retry_id = 0; $retry_id <= $this->API->settings->getAuth()->getMaxAuthTries(); $retry_id++) {
                    ...
                    $auth_key_aux_hash = \Phabel\Target\Php80\Polyfill::substr($auth_key_sha, 0, 8);
                    $new_nonce_hash1 = \Phabel\Target\Php80\Polyfill::substr(\sha1($new_nonce . \chr(1) . $auth_key_aux_hash, true), -16);
                    $new_nonce_hash2 = \Phabel\Target\Php80\Polyfill::substr(\sha1($new_nonce . \chr(2) . $auth_key_aux_hash, true), -16);
                    $new_nonce_hash3 = \Phabel\Target\Php80\Polyfill::substr(\sha1($new_nonce . \chr(3) . $auth_key_aux_hash, true), -16);
                    /*
                     * ***********************************************************************
                     * Check if the client's nonce and the server's nonce are the same
                     */
                    ...
                    switch ($Set_client_DH_params_answer['_']) {
                        case 'dh_gen_ok':
                            ...
                            $key->setServerSalt(\Phabel\Target\Php80\Polyfill::substr($new_nonce, 0, 8) ^ \Phabel\Target\Php80\Polyfill::substr($server_nonce, 0, 8));
