...
class DataCenterConnection implements JsonSerializable
{
    ...
    public function bindTempAuthKey() : \Generator
    {
        ...
        for ($retry_id_total = 1; $retry_id_total <= $this->API->settings->getAuth()->getMaxAuthTries(); $retry_id_total++) {
            try {
                ...
                $message_key = \Phabel\Target\Php80\Polyfill::substr(\sha1($encrypted_data, true), -16);
    ...
    private function syncAuthorization() : \Generator
    {
        ...
        if ($this->API->authorized === MTProto::LOGGED_IN && !$this->isAuthorized()) {
            foreach ($this->API->datacenter->getDataCenterConnections() as $authorized_dc_id => $authorized_socket) {
                ...
                if ($authorized_socket->hasTempAuthKey() && $authorized_socket->hasPermAuthKey() && $authorized_socket->isAuthorized() && $this->API->authorized === MTProto::LOGGED_IN && !$this->isAuthorized() && !$authorized_socket->isCDN()) {
                    try {
    ...
    public function signalDisconnect(int $id)
    {
        ...
        foreach ($backup as $k => $message) {
            if ($message->getConstructor() === 'msgs_state_req' || $message->getConstructor() === 'ping_delay_disconnect' || $message->isUnencrypted()) {
                unset($backup[$k]);
                continue;
            }
