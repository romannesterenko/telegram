...
abstract class DatabaseAbstract extends SettingsDatabaseAbstract
{
    ...
    {
        foreach (self::toCamel(['database', 'password', 'cache_ttl']) as $object => $array) {
    ...
    */
    ...
    {
        $this->cacheTtl = \is_int($cacheTtl) ? $cacheTtl : \Phabel\Target\Php80\Polyfill::strtotime($cacheTtl) - \time();
        return $this;
    }
    /**
     * Get password.
     ...
     * @return self
     */
    public abstract function setDatabase($database) : self;
    /**
     * Set database URI.
     *
     * @param string $uri
     * @return self
     */
    public abstract function setUri(string $uri) : self;
}