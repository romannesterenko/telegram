...
abstract class EventHandler extends InternalDoc
{
     ...
     * @return void
     */
    public static final function startAndLoop(string $session, SettingsAbstract $settings) : void
    {
     ...
     * @return void
     */
    public static final function startAndLoopBot(string $session, string $token, SettingsAbstract $settings) : void
    {
        $API = new API($session, $settings);
        $API->botLogin($token);
        $API->startAndLoop(static::class);
