...
final class WebRunner extends RunnerAbstract
{
    ...
    public static function start(string $session, int $startupId) : Promise
    {
        ...
        if (!self::$runPath) {
            ...
            if (\Phabel\Target\Php80\Polyfill::substr($uri, -1) === '/') {
                // http://example.com/path/ (assumed index.php)
                $uri .= 'index';
                // Add fake file name
            }
            $uri = \str_replace('//', '/', $uri);
            ...
            if (\Phabel\Target\Php80\Polyfill::substr($rootDir, -\strlen($uriDir)) !== $uriDir) {
            ...
            $absoluteRootDir = \Phabel\Target\Php80\Polyfill::substr($rootDir, 0, \strlen($rootDir) - \strlen($uriDir)) . DIRECTORY_SEPARATOR;
            $runPath = self::getScriptPath($absoluteRootDir);
            if (\Phabel\Target\Php80\Polyfill::substr($runPath, 0, \strlen($absoluteRootDir)) === $absoluteRootDir) {
                // Process runner is within readable document root
                self::$runPath = \Phabel\Target\Php80\Polyfill::substr($runPath, \strlen($absoluteRootDir) - 1);
            } else {
        ...
        $params = ['argv' => ['madeline-ipc', $session, $startupId], 'cwd' => Magic::getcwd()];
        $params = \http_build_query($params);
        foreach (($_SERVER['HTTPS'] ?? 'off') === 'on' ? ['tls', 'tcp'] : ['tcp', 'tls'] as $proto) {
            try {
                $address = $proto . '://' . $_SERVER['SERVER_NAME'];
                $port = $_SERVER['SERVER_PORT'];
                $res = \Phabel\Target\Php80\Polyfill::fsockopen($address, $port);
                break;
            } catch (\Throwable $e) {
