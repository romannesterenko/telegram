...
class TL
{
    ...
    public function init(TLSchema $files, array $objects = [])
    {
        ...
        foreach (\array_filter(['api' => $files->getAPISchema(), 'mtproto' => $files->getMTProtoSchema(), 'secret' => $files->getSecretSchema(), ...$files->getOther()]) as $scheme_type => $file) {
    ...
    public function serializeObject(array $type, $object, $ctx, int $layer = -1) : \Generator
    {
        ...
        switch ($type['type']) {
            ...
            case 'long':
                ...
                if (\is_string($object) && \strlen($object) === 9 && $object[0] === 'a') {
                    return \Phabel\Target\Php80\Polyfill::substr($object, 1);
                }
            ...
            case 'string':
                ...
                if ($l <= 253) {
                    ...
                    $concat .= \chr(254);
                    $concat .= \Phabel\Target\Php80\Polyfill::substr(\danog\MadelineProto\Tools::packSignedInt($l), 0, 3);
                    $concat .= $object;
        ...
        if ($bare = $type['type'] != '' && $type['type'][0] === '%') {
            $type['type'] = \Phabel\Target\Php80\Polyfill::substr($type['type'], 1);
        }
    ...
    private function deserializeInternal($stream, array &$promises, array $type)
    {
        ...
        switch ($type['type']) {
            ...
            case 'Vector t':
                ...
                switch ($constructorData['predicate']) {
                    case 'gzip_packed':
                        return $this->deserializeInternal(\gzdecode($this->deserializeInternal($stream, $promises, ['type' => 'bytes', 'connection' => $type['connection']])), $promises, ['type' => '', 'connection' => $type['connection']]);
                    case 'Vector t':
                    case 'vector':
                        break;
                    default:
        ...
        if ($type['type'] != '' && $type['type'][0] === '%') {
            $checkType = \Phabel\Target\Php80\Polyfill::substr($type['type'], 1);
        ...
        if ($constructorData['predicate'] === 'gzip_packed') {
            ...
            return $this->deserializeInternal(\gzdecode($this->deserializeInternal($stream, $promises, ['type' => 'bytes'])), $promises, ['type' => '', 'connection' => $type['connection'], 'subtype' => $type['subtype']]);
        }
        ...
        } elseif ($x['_'] === 'rpc_result' && isset($type['connection']->outgoing_messages[$x['req_msg_id']]) && isset($this->callbacks[TLCallback::METHOD_CALLBACK][$type['connection']->outgoing_messages[$x['req_msg_id']]->getConstructor()])) {
