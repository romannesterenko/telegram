...
class Conversion
{
    ...
    public static function tdesktop_fopen($fileName, $options = self::FILEOPTION_SAFE | self::FILEOPTION_USER)
    {
        ...
        foreach ($totry as $fp) {
            ...
            $md5 = \Phabel\Target\Php80\Polyfill::substr($data, -16);
            $data = \Phabel\Target\Php80\Polyfill::substr($data, 0, -16);
    ...
    public static function tdesktop_decrypt($data, $auth_key)
    {
        ...
        if ($message_key != \Phabel\Target\Php80\Polyfill::substr(\sha1($decrypted_data, true), 0, 16)) {
    ...
    const dbiKey = 0x0;
    const dbiUser = 0x1;
    const dbiDcOptionOldOld = 0x2;
    const dbiChatSizeMax = 0x3;
    const dbiMutePeer = 0x4;
    const dbiSendKey = 0x5;
    const dbiAutoStart = 0x6;
    const dbiStartMinimized = 0x7;
    const dbiSoundNotify = 0x8;
    const dbiWorkMode = 0x9;
    const dbiSeenTrayTooltip = 0xa;
    const dbiDesktopNotify = 0xb;
    const dbiAutoUpdate = 0xc;
    const dbiLastUpdateCheck = 0xd;
    const dbiWindowPosition = 0xe;
    const dbiConnectionTypeOld = 0xf;
    // 0x10 reserved
