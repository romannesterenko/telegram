...
trait MessageHandler
{
    ...
    public function encryptSecretMessage(int $chat_id, array $message, Deferred $queuePromise) : \Generator
    {
        ...
        if ($this->secret_chats[$chat_id]['mtproto'] === 2) {
            ...
            $message_key = \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($this->secret_chats[$chat_id]['key']['auth_key'], 88 + ($this->secret_chats[$chat_id]['admin'] ? 0 : 8), 32) . $message, true), 8, 16);
            list($aes_key, $aes_iv) = Crypt::aesCalculate($message_key, $this->secret_chats[$chat_id]['key']['auth_key'], $this->secret_chats[$chat_id]['admin']);
        } else {
            $message_key = \Phabel\Target\Php80\Polyfill::substr(\sha1($message, true), -16);
            list($aes_key, $aes_iv) = Crypt::oldAesCalculate($message_key, $this->secret_chats[$chat_id]['key']['auth_key'], true);
    ...
    public function handleEncryptedUpdate(array $message) : \Generator
    {
        ...
        $auth_key_id = \Phabel\Target\Php80\Polyfill::substr($message['message']['bytes'], 0, 8);
        $old = false;
        ...
        $message_key = \Phabel\Target\Php80\Polyfill::substr($message['message']['bytes'], 8, 16);
        $encrypted_data = \Phabel\Target\Php80\Polyfill::substr($message['message']['bytes'], 24);
    ...
    private function tryMTProtoV1Decrypt($message_key, $chat_id, $old, $encrypted_data)
    {
        ...
        $message_data_length = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 0, 4))[1];
        $message_data = \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 4, $message_data_length);
        if ($message_data_length > \strlen($decrypted_data)) {
            throw new \danog\MadelineProto\SecurityException(\danog\MadelineProto\Lang::$current_lang['msg_data_length_too_big']);
        }
        if ($message_key != \Phabel\Target\Php80\Polyfill::substr(\sha1(\Phabel\Target\Php80\Polyfill::substr($decrypted_data, 0, 4 + $message_data_length), true), -16)) {
    ...
    private function tryMTProtoV2Decrypt($message_key, $chat_id, $old, $encrypted_data)
    {
        ...
        $decrypted_data = Crypt::igeDecrypt($encrypted_data, $aes_key, $aes_iv);
        if ($message_key != \Phabel\Target\Php80\Polyfill::substr(\hash('sha256', \Phabel\Target\Php80\Polyfill::substr($this->secret_chats[$chat_id][$old ? 'old_key' : 'key']['auth_key'], 88 + ($this->secret_chats[$chat_id]['admin'] ? 8 : 0), 32) . $decrypted_data, true), 8, 16)) {
            throw new \danog\MadelineProto\SecurityException('Msg_key mismatch');
        }
        $message_data_length = \unpack('V', \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 0, 4))[1];
        $message_data = \Phabel\Target\Php80\Polyfill::substr($decrypted_data, 4, $message_data_length);
