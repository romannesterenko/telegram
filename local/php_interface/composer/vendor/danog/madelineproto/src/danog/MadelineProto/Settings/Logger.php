...
class Logger extends SettingsAbstract
{
    ...
    {
        $this->type = PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg' ? MadelineProtoLogger::ECHO_LOGGER : MadelineProtoLogger::FILE_LOGGER;
        Magic::start(true);
    ...
    {
        return $this->extra instanceof \Closure ? ['type', 'extra', 'level', 'maxSize'] : ['type', 'level', 'maxSize'];
    }
    /**
     * Wakeup function.
     */
    ...
    public function setType(int $type) : self
    {
        if ($type === MadelineProtoLogger::NO_LOGGER) {
            $type = PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg' ? MadelineProtoLogger::ECHO_LOGGER : MadelineProtoLogger::FILE_LOGGER;
        }
        $this->type = $type;
        return $this;
    }
    ...
    {
        return $this->type === MadelineProtoLogger::FILE_LOGGER ? Tools::absolute($this->extra) : $this->extra;
    }
    /**
    ...
    public function setExtra($extra) : self
    {
        if ($this->type === MadelineProtoLogger::CALLABLE_LOGGER && !\is_callable($extra)) {
            $this->setType(PHP_SAPI === 'cli' || PHP_SAPI === 'phpdbg' ? MadelineProtoLogger::ECHO_LOGGER : MadelineProtoLogger::FILE_LOGGER);
            return $this;
        }
        $this->extra = $extra;
        return $this;
