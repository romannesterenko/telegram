...
class Logger
{
    ...
    public function __construct(SettingsLogger $settings, string $prefix = '')
    {
        ...
        if ($this->mode === self::ECHO_LOGGER) {
            ...
            if ($maxSize !== -1) {
                ...
                $stdout =& $this->stdout;
                $this->rotateId = Loop::repeat(10 * 1000, static function () use($maxSize, $optional, &$stdout) {
        ...
        if (PHP_SAPI !== 'cli' && PHP_SAPI !== 'phpdbg') {
            try {
                \Phabel\Target\Php80\Polyfill::error_reporting(E_ALL);
                \ini_set('log_errors', "1");
                \ini_set('error_log', $this->mode === self::FILE_LOGGER ? $this->optional : Magic::$script_cwd . DIRECTORY_SEPARATOR . 'MadelineProto.log');
    ...
    public function logger($param, int $level = self::NOTICE, string $file = '') : void
    {
        ...
        if (!self::$printed) {
            ...
            $this->logger('Copyright (C) 2016-' . \Phabel\Target\Php80\Polyfill::date('Y') . ' Daniil Gentili');
        ...
        $param = Magic::$isatty ? "\x1b[" . $this->colors[$level] . 'm' . $param . "\x1b[0m" . $this->newline : $param . $this->newline;
