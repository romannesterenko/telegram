...
class RSA
{
    ...
    public static function load(TL $TL, string $rsa_key) : \Generator
    {
        ...
        $instance->fp = \Phabel\Target\Php80\Polyfill::substr(\sha1((yield from $TL->serializeObject(['type' => 'bytes'], $instance->n->toBytes(), 'key')) . (yield from $TL->serializeObject(['type' => 'bytes'], $instance->e->toBytes(), 'key')), true), -8);
        return $instance;
    }
    /**
     * Private constructor.
    ...
    {
        return \str_pad($data->powMod($this->e, $this->n)->toBytes(), 256, "\x00", STR_PAD_LEFT);
    }
}