...
trait Templates
{
    ...
    private function webEcho(string $message = '') : \Generator
    {
        ...
        if ($auth === MTProto::NOT_LOGGED_IN) {
            if (isset($_POST['type'])) {
                if ($_POST['type'] === 'phone') {
                    ...
                    $token = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginBotTokenWeb']);
                    $form = "<input type='text' name='token' placeholder='{$token}' required/>";
                }
            } else {
                $title = Lang::$current_lang['loginChoosePromptWeb'];
                $optionBot = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginOptionBot']);
                $optionUser = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginOptionUser']);
            ...
            $phone = \Phabel\Target\Php80\Polyfill::htmlentities(Lang::$current_lang['loginUserPhoneCodeWeb']);
            ...
            $hint = \Phabel\Target\Php80\Polyfill::htmlentities(\sprintf(
        ...
        $title = \Phabel\Target\Php80\Polyfill::htmlentities($title);
