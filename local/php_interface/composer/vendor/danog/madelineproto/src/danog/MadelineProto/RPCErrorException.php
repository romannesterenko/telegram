    ...
    public static function localizeMessage($method, int $code, string $error)
    {
        ...
        if (!isset(self::$errorMethodMap[$code][$method][$error]) || !isset(self::$descriptions[$error])) {
            Tools::callFork((function () use($method, $code, $error) {
                try {
                    $res = \json_decode((yield ((yield HttpClientBuilder::buildDefault()->request(new Request('https://rpc.pwrtelegram.xyz/?method=' . $method . '&code=' . $code . '&error=' . $error))))->getBody()->buffer()), true);
