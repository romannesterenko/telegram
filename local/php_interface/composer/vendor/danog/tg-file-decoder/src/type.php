...
function packTLString(string $string) : string
{
    ...
    if ($l <= 253) {
        ...
        $concat .= \chr(254);
        $concat .= \Phabel\Target\Php80\Polyfill::substr(\pack('V', $l), 0, 3);
        $concat .= $string;
...
function internalDecode(string $fileId) : array
{
    ...
    \fwrite($res, \Phabel\Target\Php80\Polyfill::substr($fileId, 8));
    \fseek($res, 0);
    $fileId = $res;
...
function internalDecodeUnique(string $fileId) : array
{
    ...
    $fileId = \Phabel\Target\Php80\Polyfill::substr($fileId, 4);
