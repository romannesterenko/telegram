...
class IntegrationTest extends TestCase
{
    ...
    public function provideFileIdsAndType() : \Generator
    {
        foreach (['CAADBAADwwADmFmqDf6xBrPTReqHFgQ', 'CAACAgQAAxkBAAIC4l9CWDGzVUcDejU0TETLWbOdfsCoAALDAAOYWaoN_rEGs9NF6ocbBA', 'CAADBAADwwADmFmqDf6xBrPTReqHAg'] as $fileId) {
            (yield ['sticker', $fileId, 'AgADwwADmFmqDQ']);
        }
        $dest = \getenv('DEST');
        $token = \getenv('TOKEN');
        foreach ($this->provideChats() as $chat) {
            ...
            (yield ['profile_photo', $result['small_file_id'], $result['small_file_unique_id']]);
            (yield ['profile_photo', $result['big_file_id'], $result['big_file_unique_id']]);
        }
        foreach ($this->provideUrls() as $type => $url) {
            if ($type === 'video_note') {
                ...
                \curl_setopt($handle, CURLOPT_POSTFIELDS, [$type => new CURLFile(\basename($url))]);
            ...
            foreach ($botResult as $subResult) {
                (yield [$type, $subResult['file_id'], $subResult['file_unique_id']]);
                if (isset($subResult['thumb'])) {
                    (yield ['thumbnail', $subResult['thumb']['file_id'], $subResult['thumb']['file_unique_id']]);
                }
            }
        }
    }
    ...
    {
        return ['sticker' => 'https://github.com/danog/MadelineProto/blob/master/tests/lel.webp?raw=true', 'photo' => 'https://github.com/danog/MadelineProto/blob/master/tests/faust.jpg', 'audio' => 'https://github.com/danog/MadelineProto/blob/master/tests/mosconi.mp3?raw=true', 'video' => 'https://github.com/danog/MadelineProto/blob/master/tests/swing.mp4?raw=true', 'animation' => 'https://github.com/danog/MadelineProto/blob/master/tests/pony.mp4?raw=true', 'document' => 'https://github.com/danog/danog.github.io/raw/master/lol/index_htm_files/0.gif', 'voice' => 'https://daniil.it/audio_2020-02-01_18-09-08.ogg', 'video_note' => 'https://daniil.it/round.mp4'];
    }
}