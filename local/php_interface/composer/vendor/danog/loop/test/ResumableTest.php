...
class ResumableTest extends Fixtures
{
    ...
    {
        $paused = $loop->resume();
        // Returned promise will resolve on next pause
    ...
    {
        $paused = $loop->resume();
        // Will resolve on next pause
        $loop->setInterval(-1);
        // Will not pause, and finish right away!
        $this->assertPreStart($loop);
        $this->assertTrue($loop->start());
        (yield delay(1));
        $this->assertFalse(self::isResolved($paused));
        // Did not pause
    ...
    public function testResumableDeferOnce(ResumableInterface $loop) : \Generator
    {
        ...
        $this->assertAfterStart($loop);
        (yield delay(1));
        $this->assertTrue(self::isResolved($paused1));
        // Did pause
        $this->assertTrue(self::isResolved($paused2));
        // Did pause
        $paused1 = $loop->resumeDeferOnce();
        $paused2 = $loop->resumeDeferOnce();
        $this->assertAfterStart($loop);
        ...
        $this->assertFinal($loop);
        (yield delay(1));
        $this->assertFalse(self::isResolved($paused1));
        // Did not pause again
        $this->assertFalse(self::isResolved($paused2));
        // Did not pause again
    }
    /**
    ...
    {
        return [[new class extends ResumableLoop implements ResumableInterface, ResumableLoopInterface
        {
            use Resumable;
