        ...
        if (count($params) === 1 && is_array($params[0]) && isset($params[0]['originalclassnamepony'])) {
            ...
            foreach ($params[0] as $key => $value) {
                if (strpos($key, chr(0) . get_class($this) . chr(0)) === 0) {
                    $key = \Phabel\Target\Php80\Polyfill::substr($key, strlen(get_class($this)) + 2);
                } elseif (strpos($key, chr(0) . '*' . chr(0)) === 0) {
                    $key = \Phabel\Target\Php80\Polyfill::substr($key, 3);
                } elseif ($key[0] === "\x00") {
                    $key = \Phabel\Target\Php80\Polyfill::substr($key, 1);
                }
